...
function removeAll(price) {
    if(confirm('Bạn chắc chắn muốn bỏ sản phẩm này')) {
        carts = carts.filter(item => item.price !== price)
        console.log(carts)
        console.log(price)
        localStorage.setItem('listCart', JSON.stringify(carts))
        renderCart()
...
function renderCart() {
    ...
    arrSetUnique.forEach(item => {
        ...
        $('.payment__tableBody').append(
            `
            <tr>
                <td><a onclick="removeAll(${item.price}); return false" href="#"><i class="fa fa-times" aria-hidden="true"></i></a></td>
    ...
    $('.sum_payment').append(
        ...
        <a href="../shopping__cart/shopping_cart_2.html" class="btn btn-danger">Thanh toán ngay</a>
    `)
}
