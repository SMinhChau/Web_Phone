    ...
    function renderBrands() {
        dataBrands.map(name => {
            $('.nav__level2--brand').append(
                `
                <li class="nav__item">
                    <a href="../list--product/list_products.html?name=${name}">${name}</a>
                </li>
                `
            )
        })
    ...
    function renderNavClassify() {
        dataClassify.vn.map(item => {
            $('.nav__level2--classify').append(
                `
                    <li class="nav__item">
                        <a href="../classify-product/classify_product.html?classify=${convertEN(item)}">${item}</a>
                    </li>
                `
            )
                ...
                <div class="item mySlide">
                    <div class="mySlide__sub">
                        <div class="mySlide__caption">
                            ...
                            href="../product/product.html?id=${item.id}"
    ...
    function renderPopulation() {
        dataPopulation.map(item => {
            $('.menu__select--population').append(
                `
                <li ><a href="../product/product.html?id=${item.id}">${item.title}</a></li>
                `
            )
        })
    }
...
function rederPavination(dataProducts) {
    let countPage = Math.ceil(dataProducts.length / MAX_products_page)
    let href = window.location.href /* countPage.com?page=3 */
    let url = new URL(href)
